...
void TIM_init(void)
{
    ...
    //TIM1所属的APB1频率为144MHz
    TIM_TimeBaseStructure.TIM_Period = 470 - 1;            //自动装载值，到该值是计时器清0，并触发中断
    ...
    TIM_Cmd(TIM1, DISABLE);                 //关闭TIM1时钟

    RCC_APB2PeriphClockCmd(RCC_APB2Periph_TIM8, ENABLE);        //启用TIM8时钟
    ...
    //TIM1所属的APB1频率为144MHz
    TIM_TimeBaseStructure1.TIM_Period = 10000 - 1;            //自动装载值，到该值是计时器清0，并触发中断
    //144MHz / 144 = 1MHz 每秒两百万，数800下才对
    TIM_TimeBaseStructure1.TIM_Prescaler = 1440 - 1;                //预分频值，首先分频为CK_PSC
    TIM_TimeBaseStructure1.TIM_CounterMode = TIM_CounterMode_Up; // 设置为向上计数模式
    TIM_TimeBaseStructure1.TIM_RepetitionCounter=0;              //溢出后立即产生中断
    TIM_TimeBaseStructure1.TIM_ClockDivision = TIM_CKD_DIV1; // 时钟分频设置，这里设为不分频，从CK_PSC分为CK_INT
    TIM_TimeBaseInit(TIM8, &TIM_TimeBaseStructure1);
    TIM_ITConfig(TIM8, TIM_IT_Update, ENABLE);
    /* 此处编写单次运行的代码(例如：初始化代码等) */

    TIM_Cmd(TIM8, DISABLE);                 //关闭TIM8时钟
    car_both_rgb_off();
}
