...
int check_rightangle_bend()
{
    ...
    else
    {
        ...
        if(line>=10&&line<=80&&Right_line[line]>=94&&Left_line[line+15]==0)
        {
            count = 0;
        ...
        else if((line>=80&&Right_line[line]>=100)&&Left_line[line+5]==0)
        {
            count = 0;
            i = line - 5;
            for(j = 0;j<188;j++)
...
if(Cross_status==0)
{
    int rightangle;
     ...
     if(check_three()&&rightangle_flag==2)//判断三岔（判断到后将triangle加一）
     {
        if ( flagLED == 0 ){
            flagLED = 101;
        }
     ...
     else if(Cross_status==0&&entered_Cross==0&&check_cross_road_out(num_white,white_line[num_white-1]))  //检测是否到达十字
     {
         flag=2;//十字直道标志位
