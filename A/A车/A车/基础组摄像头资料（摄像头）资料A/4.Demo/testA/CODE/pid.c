...
car_mode[6].PID_D=distance_PID_D;
car_mode[6].PID_I=distance_PID_I;
car_mode[6].PID_P=distance_PID_P;
car_mode[6].speed=distance_SPEED;

car_mode[7].PID_D=S_PID_D;
car_mode[7].PID_I=S_PID_I;
car_mode[7].PID_P=S_PID_P;
car_mode[7].speed=S_SPEED;
...
//    }
    if(flag==9)
    {
        ...
        motor_forward(left,car_mode[flag].speed  - 0 * (triangle_flag == 1) - 600 * (finalLow >= 10) - 1350 * (finalLow >= 10 && flag == 0) - 1300 * (count_tri >= 2 && flag != 5) - 1000 * (rightangle_flag && count_tri < 2) + Car_PID_ERROR(Lpid,-error,flag));
        motor_forward(right,car_mode[flag].speed - 0 * (triangle_flag == 1) - 600 * (finalLow >= 10) - 1350 * (finalLow >= 10 && flag == 0) - 1300 * (count_tri >= 2  && flag != 5) - 1000 * (rightangle_flag && count_tri < 2) + Car_PID_ERROR(Rpid,error,flag));
